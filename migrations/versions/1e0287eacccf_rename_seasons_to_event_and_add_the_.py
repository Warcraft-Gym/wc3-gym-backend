...
The data the rename needs: a "GNL" league that every existing event joins, a
"KOTH" league that waits for its events, and one round-robin stage per event
carrying the event's map rules, so the GNL rounds have a stage to hang off
in C1.

Revision ID: 1e0287eacccf
Revises: f4b7c02e9a15
...
down_revision: str | Sequence[str] | None = "f4b7c02e9a15"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None

ENUM_VALUES = {
    "entrantkind": ("solo", "drafted_teams"),
    "leaguekind": ("gnl", "koth", "custom"),
...
def event_columns() -> list[sa.Column]:
    ...
    return [
        ...
        sa.Column(
            "checkin_enabled", sa.Boolean(), nullable=False, server_default=sa.true()
        ),
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("page_url", sa.String(length=500), nullable=True),
        sa.Column(
            "kind", enum_type("leaguekind"), nullable=False, server_default="custom"
        ),
        sa.Column(
            "entrant_kind",
    ...
    # Every season that exists today is a GNL season. KOTH gets its league
    # here too; C1 dates its nights and gives it an event.
    op.execute(
        "INSERT INTO league (name, short_name, kind, entrant_kind) VALUES "
        "('GNL', 'GNL', 'gnl', 'drafted_teams'), ('KOTH', 'KOTH', 'koth', 'solo')"
    )
    op.execute(
