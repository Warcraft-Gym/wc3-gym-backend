...
def login():
    ...
    if data['token'] == os.getenv('ADMIN_TOKEN'):
        access_token = create_access_token(identity='admin',expires_delta=timedelta(minutes=token_time))
        refresh_token = create_refresh_token(identity='admin',expires_delta=timedelta(minutes=refresh_token_time))
...
def refresh():
    ...
    new_access_token = create_access_token(identity=current_user, expires_delta=timedelta(minutes=token_time))
