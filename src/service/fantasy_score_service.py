...
class FantasyScoreAppService:
        ...
        for week in range(1, season.number_weeks + 1):
            ...
            for series in season_week_series:
                if series.player1_score == 2:
                    week_race_wins[series.player1.race] = week_race_wins.get(series.player1.race, 0) + 1
                    week_race_looses[series.player2.race] = week_race_looses.get(series.player2.race, 0) + 1
                elif series.player2_score == 2:
                    week_race_wins[series.player2.race] = week_race_wins.get(series.player2.race, 0) + 1
                    week_race_looses[series.player1.race] = week_race_looses.get(series.player1.race, 0) + 1
            ...
            if has_played_games:
                ...
                sorted_races = sorted(week_result.items(), key=lambda item: (item[1], str(item[0])), reverse=True)
                ...
                for race, ratio in sorted_races:
                    ...
                    if prev_ratio is not None and ratio != prev_ratio:
                        ...
                        for prev_race in races_at_current_rank:
                            race_points[prev_race] = race_points.get(prev_race, 0) + points_to_award
    ...
    def _calculate_fantasy_team_scores(self, fantasy_team, season, race_points, include_breakdown=False):
        """
                    ...
                    if not week_player_series:
                        ...
                        for series in week_player_series:
                            if series.player1_score is not None and series.player2_score is not None:
                                ...
                                player_score = series.player1_score if is_player1 else series.player2_score
                                opponent_score = series.player2_score if is_player1 else series.player1_score
                                
                                points = self.calculatePoints(player_score, opponent_score)
                                week_points += points
                                
                                if include_breakdown:
                                    opponent_name = series.player2.name if is_player1 else series.player1.name
        ...
        # Bet points
        series_q_string = f"user_id=={fantasy_team.captain.id} and season_id=={season.id}"
        series_query = QueryUtil.parseQuery(series_q_string)
    ...
    def calculateTeamScores(self, season):
        ...
        if fteams:
            for fteam in fteams:
                ...
                scores = self._calculate_fantasy_team_scores(fteam, season, race_points, include_breakdown=False)
    ...
    def getTeamScoreBreakdown(self, fantasy_team_id, season):
        ...
        race_points, race_stats, race_weekly_details = self._calculate_race_points(season, include_weekly_details=True)
        
        # Calculate all score components using shared method
        scores = self._calculate_fantasy_team_scores(fantasy_team, season, race_points, include_breakdown=True)
