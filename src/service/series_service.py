...
class SeriesAppService:
    def __init__(self, series_service: SeriesDBService, score_app_service: ScoreAppService, user_app_service: UserAppService):
    ...
    def updateGNLSeasonStats(self, series):
        p1_season_data = self.calculateUserSeasonStats(series.player1.id, series.match.season_id, series.match.team1_id)
        self.user_app_service.updateUserTeamSeasonStats(p1_season_data)
        p2_season_data = self.calculateUserSeasonStats(series.player2.id, series.match.season_id, series.match.team2_id)
