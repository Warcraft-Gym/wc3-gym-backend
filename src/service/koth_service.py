...
class KothAppService:
    ...
    # ============ Signup Methods ============
    def create_signup_from_twitch(self, twitch_username: str, battle_tag: str, preferred_race: str = None):
        """
        ...
        if preferred_race:
            ...
            if not signup_race:
                raise ValueError(f"Invalid race '{preferred_race}'. Valid options: orc, human, undead, nightelf, random")
        ...
        try:
            ...
            raise Exception(f"Failed to validate W3C stats for {battle_tag}: {str(e)}")

        # Determine final race and MMR
        if signup_race:
            # Race was specified (including RANDOM)
            if signup_race not in race_mmr_data:
                raise Exception(f"No W3Champions statistics found for {battle_tag} with race {signup_race} in the last 3 seasons")
            avg_mmr = race_mmr_data[signup_race]
            final_race = signup_race
        else:
            # No race specified - find race with highest MMR
            if not race_mmr_data:
                raise Exception(f"No valid MMR data found for {battle_tag} in the last 3 seasons")
    ...
    def create_match(self, match: KothMatchDTO, participant_signup_ids: list):
        ...
        for participant in participant_signup_ids:
            ...
            if not signup:
                raise NotFoundException(f"Signup not found by Id: {participant['signup_id']}")
        ...
        if len(unique_teams) != match.num_teams:
            raise ValueError(f"Expected {match.num_teams} teams, but participants are assigned to {len(unique_teams)} teams")
    ...
    def update_match_result(self, match_id: int, winner_team_number: int):
        ...
        if winner_team_number < 1 or winner_team_number > match.num_teams:
            raise ValueError(f"Winner team number must be between 1 and {match.num_teams}")
    ...
    def _get_w3c_stats_for_season(self, w3c_service: W3CService, battle_tag: str, season: int):
