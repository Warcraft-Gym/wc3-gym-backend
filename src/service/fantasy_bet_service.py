...
class FantasyBetAppService:
    def __init__(self, fantasy_bet_service: FantasyBetDBService, settings_app_service=None):
    ...
    def _apply_bet_points_logic(self, bet: FantasyBetDTO):
        ...
        try:
            ...
            fixed_bet_points_setting = self.settings_app_service.get_setting('fantasy_fixed_bet_points')
            use_fixed_points = fixed_bet_points_setting and fixed_bet_points_setting.get('value', '').lower() == 'true'
            
            if use_fixed_points:
                # Use the fixed bet points value from settings
                bet_points_value_setting = self.settings_app_service.get_setting('fantasy_bet_points_value')
                if not bet_points_value_setting or not bet_points_value_setting.get('value'):
                    raise ValueError("Fixed bet points enabled but fantasy_bet_points_value is not configured")
                
                bet.bet_points = int(bet_points_value_setting.get('value'))
            else:
                # Validate that bet_points were provided from UI
                if bet.bet_points is None or bet.bet_points <= 0:
                    raise ValueError("bet_points is required when fixed bet points is disabled")
                
                # Validate min/max bet points only if they are defined and different
                try:
                    min_bet_setting = self.settings_app_service.get_setting('fantasy_min_bet_points')
                    min_bet = int(min_bet_setting.get('value')) if min_bet_setting and min_bet_setting.get('value') else None
                except (NotFoundException, Exception):
                    min_bet = None
                
                try:
                    max_bet_setting = self.settings_app_service.get_setting('fantasy_max_bet_points')
                    max_bet = int(max_bet_setting.get('value')) if max_bet_setting and max_bet_setting.get('value') else None
                except (NotFoundException, Exception):
                    max_bet = None
