...
GNL_RATING_DECAY_RATE_PER_SEASON = 0.15  # Every season remove 15% of each players' points
GNL_RATING_FLAT_MULTIPLIER = 100.0  # Creates separation between scores

class PlayerCareerStatsAppService:
    def __init__(self, stats_db_service: PlayerCareerStatsDBService, series_service: SeriesDBService):
        self.stats_db_service = stats_db_service
        self.series_service = series_service
    
    def get_all_career_stats(self):
        """Get all player career stats ordered by rating"""
        return self.stats_db_service.get_all()
    
    def get_career_stats_by_user(self, user_id: int):
        """Get career stats for a specific user"""
        return self.stats_db_service.get_by_user_id(user_id)
    ...
    def recalculate_all_stats(self):
        ...
        for series in all_series:
            ...
            if player1_id:
                if player1_id not in series_by_player:
                    ...
                    if series.player1:
                        player_names[player1_id] = series.player1.name or series.player1.w3c_name or 'Unknown'
            ...
            if player2_id:
                if player2_id not in series_by_player:
                    ...
                    # Get player name from series DTO
                    if series.player2:
                        player_names[player2_id] = series.player2.name or series.player2.w3c_name or 'Unknown'
        ...
        all_existing_stats = self.stats_db_service.get_all()
        
        # Create lookup dictionaries for fast access
        stats_by_user_id = {stat.user_id: stat for stat in all_existing_stats if stat.user_id}
        stats_by_name = {stat.player_name: stat for stat in all_existing_stats if stat.player_name}
        ...
        for user_id, user_series in series_by_player.items():
            ...
            if not existing_stats:
                ...
                if existing_stats:
                    logger.info(f"Found unmapped historical record for '{player_name}', linking to user_id {user_id}")
            ...
            current_stats = self._calculate_player_stats_from_series(user_id, user_series)
            
            # Combine historical + current for totals
            total_series_won = historical_baseline['series_won'] + current_stats['series_won']
            total_series_lost = historical_baseline['series_lost'] + current_stats['series_lost']
            total_games_won = historical_baseline['games_won'] + current_stats['games_won']
            total_games_lost = historical_baseline['games_lost'] + current_stats['games_lost']
            
            # Calculate winrates
            total_series = total_series_won + total_series_lost
            series_winrate = (total_series_won / total_series * 100) if total_series > 0 else 0.0
            
            total_games = total_games_won + total_games_lost
            games_winrate = (total_games_won / total_games * 100) if total_games > 0 else 0.0
            
            # Get unique seasons played
            seasons_in_data = len(set(s.match.season_id for s in user_series if s.match and s.match.season_id))
            ...
            final_rating = self._calculate_gnl_rating(user_id, user_series, historical_baseline['rating'], all_system_seasons)
            logger.debug(f"Calculated GNL rating for user {user_id} ({player_name}): {final_rating}")
            # Add to updates
            updates.append({
                'user_id': user_id,
                'player_name': player_name,
        ...
        for historical_stat in all_existing_stats:
            ...
            has_historical_data = (
                (historical_stat.historical_rating and historical_stat.historical_rating > 0) or
                (historical_stat.historical_series_won and historical_stat.historical_series_won > 0) or
                (historical_stat.historical_series_lost and historical_stat.historical_series_lost > 0) or
                (historical_stat.historical_games_won and historical_stat.historical_games_won > 0) or
                (historical_stat.historical_games_lost and historical_stat.historical_games_lost > 0) or
                (historical_stat.historical_seasons_played and historical_stat.historical_seasons_played > 0)
            )
            ...
            logger.debug(f"Decayed historical rating for user {historical_stat.user_id} ({historical_stat.player_name}): {decayed_rating}")
            ...
            series_winrate = (historical_series_won / total_series * 100) if total_series > 0 else 0.0
            
            total_games = historical_games_won + historical_games_lost
            games_winrate = (historical_games_won / total_games * 100) if total_games > 0 else 0.0
        ...
        # Batch update via DB service
        result = self.stats_db_service.batch_update_stats(updates)
    ...
    def _calculate_gnl_rating(self, user_id, series_list, historical_rating=0, all_system_seasons=None):
        ...
        if series_by_season:
            ...
            for season_id in seasons:
                ...
                for series in series_by_season[season_id]:
                    ...
                    match_rating = GNL_RATING_MATCH_WIN_VALUE if is_victory else GNL_RATING_MATCH_LOSS_VALUE
        ...
        gnl_rating = historical_rating / GNL_RATING_FLAT_MULTIPLIER if historical_rating > 0 else 0
    ...
    def update_career_stats(self, stat_id: int, stat_dto):
        ...
        with self.stats_db_service.get_session() as session:
            updated_stat = DBPlayerCareerStats.update(session, stat_id, **stat_dto.to_db_dict())
            if not updated_stat:
                return None
    ...
    def delete_career_stats(self, stat_id: int):
        ...
        stat = self.stats_db_service.get(stat_id)
        if not stat:
            return False
        
        self.stats_db_service.delete(stat_id)
        return True
