...
def test_a_negative_map_score_answers_422(
    client: Client, auth_headers: dict[str, str], seeded: dict[str, Any]
) -> None:
    """A score below zero is the caller's fault, so the write model refuses it
    before it is stored; the season's own cap is SeriesService's 400."""
    resp = client.put(
