...
def test_the_seasons_become_events_of_the_gnl_league(tmp_path: Path) -> None:
    ...
    with engine.connect() as connection:
        ...
        assert connection.execute(
            text("SELECT id, name, kind, entrant_kind FROM league ORDER BY id")
        ).all() == [(1, "GNL", "gnl", "drafted_teams"), (2, "KOTH", "koth", "solo")]
        assert connection.execute(
            text(
...
def test_the_seasons_view_is_dropped_and_comes_back_on_downgrade(
    ...
    """The rename leaves the view for the running deploy; this drop takes it away."""
