...
The production build runs `alembic upgrade head` (`vercel.json`) before the new code is promoted, so a migration that fails stops the deploy. A preview build gets its own database on the staging Supabase project: `api/preview_db.py` copies `wc3gym_base` (a seeded, migrated template) to `wc3gym_pr<N>` and migrates it, and `api/index.py` points the preview at it. The preview `DB_URL` in the Vercel project names the staging project. The `Drop preview databases` workflow removes a database when its pull request closes and sweeps weekly for branches that are gone (`scripts/drop_previews.py`, secret `STAGING_DB_URL`). The old code keeps serving while the build runs, so every migration must work with the code before it and after it: add columns nullable or with a default, drop a column only after the code that read it has shipped.
