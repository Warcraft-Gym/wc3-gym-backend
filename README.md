...
Two switches are columns on the season, not settings: `signups_open` (off, a signup is a request an admin may grant) and `scheduling_enabled` (off, the event takes no availability answers and a write answers 403 `scheduling_disabled`). A season's phase is derived from its series and never stored. An availability answer is one row per player per round of a season (`round_availability`, still keyed by `playday` beside the new `round_id`), and the matching `event_round` row carries that round's number, date window and the map of game 1; `season_rounds` and `user_season_availability` are views over the new tables for the deploy that still reads them. `user_block` and `user_busy` are the player's own soft hints: they belong to the player rather than to a season, and they never write the per-round answer.
...
| `DEV_ALERTS_WEBHOOK_URL` | Optional Discord channel webhook (TEST guild #webhooks); the egress snapshot posts there when a day is over budget or the run fails. Unset, nothing posts | `https://discord.com/api/webhooks/<id>/<token>` |
