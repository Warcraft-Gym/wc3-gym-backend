...
- A series row of a finished event carries the MMR of the time per side: `mmr_at` at the series time, else its round's first day, on the race the row names, inside the event's W3Champions seasons, one statement per finished event and no ladder row sent. No match in bound is null, never today's figure. A running event's rows read `w3cstats`. The draft board's meetings read `mmr_at` inside each meeting's own event, plus the current W3Champions season while that event's end has not passed; the fantasy tiers read `mmr_on` on the Apply date inside the season's W3Champions seasons, else the current one; a Discord series card reads `mmr_on` now inside the current and the previous W3Champions season.
