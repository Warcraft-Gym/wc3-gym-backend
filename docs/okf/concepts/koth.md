...
A night is an event of the KOTH league: one stage of format `koth`, best of one, and three divisions that are the brackets, opened at three MMR bounds. Tonight is the newest published KOTH event that no admin closed, whether its signups stand open or not. Nothing stores "tonight". One night is open at a time: `POST /koth/nights` answers 409 while another KOTH night that is not archived has no `closed_at`. A night ends only when an admin closes it: the close deletes the series nobody played and stamps `event.closed_at`, and the stamp is what makes the night read finished. A chain with every series scored and no stamp reads running, and a closed night grows no chain.
...
The doors that name no night, `GET /koth/signup` and the old `/koth/signups` routes, enter tonight and answer 400 "Signups are closed" while its signups stand closed; the withdraw by battle tag does the same.
