...
type: Data Model
title: egress_ledger
description: "What each route cost the database, one row per day, route and method: calls, statements, rows read and response bytes."
...
| `rows` | BIGINT | no | Rows the database reported for those statements. |
