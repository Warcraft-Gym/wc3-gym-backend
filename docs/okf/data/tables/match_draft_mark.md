...
| `ready_by_user_id` | INTEGER | yes | The captain who marked the team ready; null when no mark stands or an admin token set it. |
| `ready_at` | TIMESTAMP | yes | When the mark was set, and the mark itself: the team is ready when it holds a time. |
