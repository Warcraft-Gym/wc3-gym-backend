...
| The workbook import | any tag, without case; a blank or stand-in tag by `name`, without case, among the people with no tag row | the workbook row's fields, and a tag row with source `sheet` for a real tag |
...
- Only `GET /users/{key}` read by a logged-in caller serves `discordTag` and `discordId` (`UserMemberPublic`). Every other read, and that read for an anonymous caller, leaves them out, a user nested in another answer included; the Discord cards read them from the in-memory shape.
