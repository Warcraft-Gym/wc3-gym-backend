...
|---|---|---|---|
| `key` | TEXT | no | The check: `egress` for the egress snapshot, `db_size` for the database size. |
| `level` | TEXT | no | `normal`, `amber`, `red` or `unavailable`; `db_size` is `normal` or `red`. |
