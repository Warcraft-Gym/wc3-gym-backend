...
* [Scheduled jobs](jobs.md) - Three job routes behind a shared secret, one called daily by Vercel, one every five minutes by a Cloudflare Worker, because the hosting plan allows one cron a day, and one that reads the egress ledger.
