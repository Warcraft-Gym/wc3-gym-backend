...
* **Update**: `discordTag` and `discordId` leave every user answer but a logged-in `GET /users/{key}`, and `discord_id` leaves the cast answer; an anonymous `GET /users/{key}` carries a public cache header.
